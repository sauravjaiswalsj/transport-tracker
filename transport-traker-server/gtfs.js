...
exports.GTFS = class {
  ...
  getStopTimes() {
	...
	getStopsForTrip(trip_id){
		return this.db.allAsync(` SELECT st.arrival_time as arrival_time, st.departure_time as departure_time,
               st.stop_id as stop_id, st.stop_sequence as stop_sequence,
               s.stop_lat as lat, s.stop_lon as lng, s.stop_name as stop_name, cd.date as date
        FROM stop_times as st INNER JOIN stops AS s INNER JOIN calender_dates AS cd INNER JOIN trips AS t WHERE st.trip_id = $trip_id
			AND st.stop_id = s.stop_id
         		 AND st.trip_id = t.trip_id
         		 AND t.service_id = cd.service_id
       			 ORDER BY departure_time`,
